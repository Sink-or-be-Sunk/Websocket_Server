...
describe("Handle Lobby Requests ", () => {
    ...
    it("Allow Player 1 to change game type", () => {
        const obj = { req: REQ_TYPE.GAME_TYPE, id: "one", data: GAME_TYPE.BASIC };
        const str = JSON.stringify(obj);
        const msg = new WSClientMessage(str);
        const responses = lobby.handleReq(msg);
        const results = [
            new WSServerMessage({ header: SERVER_HEADERS.GAME_TYPE_APPROVED, at: obj.id }),
            new WSServerMessage({ header: SERVER_HEADERS.GAME_TYPE_APPROVED, at: "two", meta: obj.data }),
        ];
        for (let i = 0; i < results.length; i++) {
            const result = results[i];
            const resp = responses[i];
