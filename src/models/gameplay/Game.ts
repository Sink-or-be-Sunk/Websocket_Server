...
export class Game {
	...
	constructor(id: string, socket: WebSocket, type: GAME_TYPE) {
		this.id = id;
	 ...
	 */
	makeMove(id: string, moveRaw: string): Response {
		if (this.players[this.turn].id == id) {
			if (this.isStarted()) {
				...
				if (move.isValid(this.rules.boardSize)) {
					...
					if (board) {
						...
						if (res.valid) {
							...
							if (
								res.meta.includes(ResponseHeader.GAME_OVER)
							) {
				...
				return new Response(
					false,
					ResponseHeader.GAME_NOT_STARTED,
				);
			}
		} else {
	...
	positionShips(id: string, positionsRaw: string): Response {
		if (this.isStarted()) {
			return new Response(
				false,
				ResponseHeader.GAME_IN_PROGRESS,
			);
		} else {
			...
			if (layout.type == LAYOUT_TYPE.VALID) {
				...
				if (board) {
					...
					return new Response(
						false,
						ResponseHeader.BOARD_NOT_FOUND,
					);
				}
			} else {
